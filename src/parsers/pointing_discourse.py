...
from src.utils.transform import DiscourseTreeDoc
...
class PointingDiscourseParser(Parser):
    ...
    def _evaluate(self, loader):
        ...
        for words, feats, edu_break, golden_metric, (spans, labels), parsing_orders in loader:
            ...
            preds=[[(i, k, j, self.CHART.vocab.itos[label])
                   ...
                   for pred in preds]
            total_loss += loss.item()
            metric([DiscourseTreeDoc.build(tree)
                    for tree in preds],
                   [DiscourseTreeDoc.build_gold(_edu_break, _golden_metric)
    ...
    def _predict(self, loader):
        ...
        for words, feats, edu_break, golden_metric, (spans, labels), parsing_orders in loader:
            ...
            preds['trees'].extend( [DiscourseTreeDoc.build(
                [(i, k, j, self.CHART.vocab.itos[label])
                 for i, k, j, label in pred])
                for pred in preds_])
            # preds['gold_trees'].extend([DiscourseTreeDoc.build_gold(_edu_break, _golden_metric)
            #         for _edu_break, _golden_metric in zip(edu_break,golden_metric)])
    ...
    def build(cls, path, min_freq=2, fix_len=20, data_path='', **kwargs):
        ...
        if args.feat in ('char', 'bert'):
            transform = DiscourseTreeDoc(WORD=(WORD, FEAT), EDU_BREAK=EDU_BREAK, GOLD_METRIC=GOLD_METRIC, CHART=CHART, PARSINGORDER=PARSINGORDER)
        # else:
