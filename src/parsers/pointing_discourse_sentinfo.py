...
from src.models import PointingDiscourseSentinfoModel
...
from src.utils.transform import DiscourseTreeDocSentinfo
...
class PointingDiscourseSentinfoParser(Parser):

    NAME = 'pointing-discourse'
    MODEL = PointingDiscourseSentinfoModel
    ...
    def _train(self, loader):
        ...
        total_loss=0
        for words, feats, original_edu_break, golden_metric, sent_break, edu_break, (spans, labels), gold_parsing_orders, parsing_orders in bar:
            # print(sent_break[0])
            # print(edu_break[0])
    ...
    def _evaluate(self, loader):
        ...
        for words, feats, original_edu_break, golden_metric, sent_break, edu_break, (spans, labels), gold_parsing_orders, parsing_orders in loader:
            # batch_size, seq_len = words.shape
            # lens = words.ne(self.args.pad_index).sum(1) - 1
            # mask = lens.new_tensor(range(seq_len - 1)) < lens.view(-1, 1, 1)
            # mask = mask & mask.new_ones(seq_len-1, seq_len-1).triu_(1)
            ...
            loss = self.model.loss(words, feats, spans, labels, parsing_orders)
            preds = self.model.decode(words, feats, sent_break)
            ...
            preds=[[(i, k, j, self.CHART.vocab.itos[label])
                   ...
                   for pred in preds]
            total_loss += loss.item()
            metric([DiscourseTreeDocSentinfo.build(tree)
                    for tree in preds],
                   [DiscourseTreeDocSentinfo.build_gold(_edu_break, _golden_metric)
    ...
    def _predict(self, loader):
        ...
        # for words, feats, golden_metric, sent_break, edu_break, (spans, labels), gold_parsing_orders, parsing_orders in loader:
        for words, feats, original_edu_break, golden_metric, sent_break, edu_break, (spans, labels), gold_parsing_orders, parsing_orders in loader:
            # batch_size, seq_len = words.shape
            # lens = words.ne(self.args.pad_index).sum(1) - 1
            # mask = lens.new_tensor(range(seq_len - 1)) < lens.view(-1, 1, 1)
            ...
            preds_ = self.model.decode(words, feats, sent_break)
            preds['trees'].extend( [DiscourseTreeDocSentinfo.build(
                [(i, k, j, self.CHART.vocab.itos[label])
                 for i, k, j, label in pred])
                for pred in preds_])
            # preds['gold_trees'].extend([DiscourseTreeDocSentinfo.build_gold(_edu_break, _golden_metric)
    ...
    def build(cls, path, min_freq=2, fix_len=20, data_path='', **kwargs):
        ...
        GOLDPARSINGORDER = ParsingOrderField('gold_parsingorder')
        if args.feat in ('char', 'bert'):
            transform = DiscourseTreeDocSentinfo(WORD=(WORD, FEAT), ORIGINAL_EDU_BREAK = ORIGINAL_EDU_BREAK,GOLD_METRIC=GOLD_METRIC, SENT_BREAK=SENT_BREAK, EDU_BREAK=EDU_BREAK,
                                                 CHART=CHART, GOLDPARSINGORDER=GOLDPARSINGORDER, PARSINGORDER=PARSINGORDER)
        # else:
